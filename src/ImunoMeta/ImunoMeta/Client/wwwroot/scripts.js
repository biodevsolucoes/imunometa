...
window.loadGoogleMapsScript = function () {
    return new Promise((resolve, reject) => {
        if (window.google && window.google.maps) {
            ...
            script.src = "https://maps.googleapis.com/maps/api/js?key=AIzaSyCgafNSNfj4jBAqnBZiM5mDrE2_sphdNT8&libraries=places";
            script.async = true;
            script.defer = true;
...
window.initializeMap = function (latitude, longitude, locais) {
    ...
    locais.forEach(local => {
        ...
        marcador.addListener('click', function () {
            ...
            img.setAttribute("src", `https://maps.googleapis.com/maps/api/staticmap?center=${local.latitude},${local.longitude}&zoom=12&size=300x300&markers=color:red|label:A|${local.latitude},${local.longitude}&key=AIzaSyCgafNSNfj4jBAqnBZiM5mDrE2_sphdNT8`)
